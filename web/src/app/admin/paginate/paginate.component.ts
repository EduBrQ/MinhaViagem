...
export class PaginateComponent implements OnInit {
  ...
  public static readonly TOTAL_PAGS_PADRAO: number = 2;
  ...
  ngOnInit() {
    this.qtdAdjacentes = this.qtdAdjacentes || PaginateComponent.ADJACENTES_PADRAO;
    this.qtdPorPagina = this.qtdPorPagina || PaginateComponent.TOTAL_PAGS_PADRAO;
    this.pagina = +this.route.snapshot.queryParams['pagina'] || PaginateComponent.PAG_PADRAO;
    this.totalRegistros = this.totalRegistros || PaginateComponent.REG_PADRAO;
    this.qtdPaginas = Math.ceil(this.totalRegistros / this.qtdPorPagina);

    console.log('toto', this.qtdPorPagina)
    this.gerarLinks();
  }
